...
export default function home() {
  ...
  const slidetext = [
    {
      src: "The UNHCR Bear auction is now live!  All proceeds directly support the emergency relief appeal and bring hope to those affected by the earthquakes.",
    },
    {
      src: "Royale 2 is Cymbal's second on-chain event with 1,000 winners and $25,000 up for grabs! Each day, tickets are eliminated at random — until one remains. The last one standing wins $12,500.",
    },
    {
      src: "Hey Creators! Are you set up in Creator Hub yet? Learn about the benefits &amp; products you can access in Creator Hub and how to get onboarded!",
    },
    {
      src: "Building a decentralized network to unite like-minded people globally, creating a strong brand for our common goal of emancipating from the current system.",
    },
  ];
  const trends = [
    {
  ...
  const Sliderouter = [
    ...
    {
      src: "/Resources/MagicDAO",
    },
    {
      src: "/Launchpad/launches",
    },
    {
      src: "/dashboard",
    },
      ...
      <Box w="calc(100vw - 80px)" ml="80px" mt="80px" pos="relative">
            ...
            {Slide.map((item, key) => (
              <Box w="70rem" ml="8rem">
                <Box
                  ...
                  alignItems={"center"}
                >
                  <Box padding={"2rem"} w="30rem">
                    <Heading fontWeight={900} size="2xl" mb="1rem">
                      {Slide[key].src}
                    </Heading>
                    <Heading variant="h5" opacity={0.6}>
                      {slidetext[key].src}
                    </Heading>
                    <Button
                      variant={"first"}
                      w="9rem"
                      mt="2rem"
                      onClick={() => router.push(Sliderouter[key].src)}
                    >
                      {SlideButton[key].src}
                    </Button>{" "}
                  </Box>
          ...
          >
            {trends.map((item, key) => (
              <Box
                ...
                alignItems={"center"}
              >
                <Box w="30px" h="30px" overflow="hidden" rounded={"100%"}>
                  <Image src={"/trend.jpeg"} alt="" height={30} width={30} />
                </Box>
                <Box w="10rem" ml="1rem">
                  {trends[key].src}
                </Box>
                ...
                <Box color="#e42575" w="7rem">
                  <Link href="/Resources/MagicDAO">View Details</Link>
                </Box>
                <Center
                  ml="1rem"
                  bgColor={"#ffffff10"}
        ...
        <Box padding={"0 1rem"} mt="6rem" display="flex" alignItems={"center"}>
          ...
          <Box w="10%" display={"flex"} justifyContent="flex-end">
            <Button variant={"second"}>
              <Link href="/marketplace/popular">See All</Link>
            </Button>
          </Box>
        </Box>
        ...
        <Box padding={"1.5rem"} display="flex" alignItems={"center"}>
          <Box w="90%" display={"flex"}>
            <Heading>Launchpad drops</Heading>
            <Center
              border="1px solid #ffffff10"
              rounded={"2rem"}
              ml="20px"
          ...
          <Center justifyContent={"flex-end"} w="10%">
            <Button
              variant={"fourth"}
              border="1px solid #ffffff10"
              ml="10px"
              onClick={() => {
                setNow(!now);
                // router.push("/Launchpad/launches");
              }}
            >
              <Heading size="sm">
                <Link href="/Launchpad/launches">See All</Link>
              </Heading>
            </Button>
          </Center>
        </Box>
